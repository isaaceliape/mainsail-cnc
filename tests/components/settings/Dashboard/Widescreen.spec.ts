...
describe('SettingsDashboardTabWidescreen.vue', () => {
    it('renders without crashing', () => {
        const wrapper = mount(SettingsDashboardTabWidescreen, { global: { plugins: [store, i18n] } })
        expect(wrapper.exists()).toBe(true)
    })

    it('renders three sortable columns', () => {
        const wrapper = mount(SettingsDashboardTabWidescreen, { global: { plugins: [store, i18n] } })
        expect(wrapper.findAll('.dashboard-sortable').length).toBe(3)
    })
})

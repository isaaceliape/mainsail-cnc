...
const t = (key: string): string => {
    const translations: Record<string, string> = {
        'Files.DuplicateFile': 'Duplicate File',
        'Files.Name': 'Name',
...
describe('GcodefilesDuplicateFileDialog.vue', () => {
    beforeEach(() => {
        vi.clearAllMocks()
    })
    ...
    it('pre-fills input with current filename after opening', async () => {
    ...
    it('rename button is disabled when name is unchanged', async () => {
        const wrapper = mount(GcodefilesDuplicateFileDialog, {
            props: { modelValue: false, item: sampleFile },
            global: mountOptions.global,
        })
        ...
        const duplicateButton = buttons[buttons.length - 1]
        expect(duplicateButton.props('disabled')).toBe(true)
    })

    it('dispatches server.files.copy on duplicate', async () => {
        const wrapper = mount(GcodefilesDuplicateFileDialog, {
            props: { modelValue: false, item: sampleFile },
            global: mountOptions.global,
        })
        ...
        await inputEl.setValue('duplicated-file.gcode')
        ...
        expect(mockSocketEmit).toHaveBeenCalledWith('server.files.copy', {
            source: 'gcodesgcodes/subdir/test-file.gcode',
            dest: 'gcodesgcodes/subdir/duplicated-file.gcode',
        })
    })
